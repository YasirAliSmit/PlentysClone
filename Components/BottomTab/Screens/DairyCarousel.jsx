...
// import {fetchDairyProducts} from '../../../redux/AllAction';
import {find} from 'lodash';
const Product = ({title, id}) => {
  const [uiData, setUiData] = useState([]);
  const dispatch = useDispatch();
  const products = useSelector(state => state.main.dairyCarousel);
  const navigation = useNavigation();
  const cartProducts = useSelector(state => state.main.cartItems);
  // useEffect(() => {
  //   dispatch(fetchDairyProducts(id));
  // }, []);
  //console.log('this is for dairy', id);
